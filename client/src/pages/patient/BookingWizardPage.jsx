 ...
 *   ראו services/appointmentService + services/doctorService.
 */
export function BookingWizardPage() {
  ...
  // בחירה שמאפסת את השלבים שאחריה כדי לשמור על עקביות
  ...
  const confirm = () => navigate(PATHS.appointments); // דמו: לאחר קביעה → רשימת התורים
      ...
      <div className={styles.body}>
        <div className={styles.card}>
              ...
              <div className={styles.deptGrid}>
                {departments.map((d) => (
                  <button
                    key={d.id}
                    type="button"
                    className={`${styles.deptCard} ${departmentId === d.id ? styles.selected : ""}`}
              ...
              <div className={styles.doctorList}>
                {doctorList.map((d) => (
                  <button
                    key={d.id}
                    type="button"
                    className={`${styles.doctorCard} ${doctorId === d.id ? styles.selected : ""}`}
          ...
          {step === 2 && (
            <section className={styles.step}>
              ...
              <p className={styles.stepSub}>
                {doctor?.name} · ימים זמינים מסומנים
              </p>
              <div className={styles.calendar}>
                ...
                <div className={styles.calLegend}>
                  <span>
                    <span className={`${styles.legendSwatch} ${styles.swatchFree}`} />פנוי
                  </span>
                  <span>
                    <span className={`${styles.legendSwatch} ${styles.swatchSelected}`} />נבחר
                  </span>
                  <span>
                    <span className={`${styles.legendSwatch} ${styles.swatchFull}`} />מלא/סגור
                  </span>
                </div>
              </div>
            </section>
          ...
          <div className={styles.nav}>
            ...
            {step < 4 ? (
              <Button onClick={next} disabled={!canContinue} iconStart={<Icon name="chevronLeft" size={16} stroke={2.4} />}>
                {CONTINUE_LABEL[step]}
              </Button>
            ) : (
